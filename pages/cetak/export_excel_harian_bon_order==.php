...
// I: ≤ today & creation > 2025-06-01, irisan approved, exclude OK/MU + rincian (SO, OrderLine, AKJ, STATUS)
...
if (!empty($codesI) && !empty($codesApprovedMy)) {
  ...
  while ($p = db2_fetch_assoc($resPairsI)) {
    ...
    $ol  = (int)$p['ORDERLINE'];
    $akj = trim((string)($p['AKJ'] ?? ''));
    if (!isset($exSet[$so.'|'.$ol])) {
      $I++;
      $I_details[] = [$so, $ol, $akj];
    }
  }
...
/* === Registrasi semua string SEBELUM membuat sharedStrings.xml === */
...
/* Judul & header tabel rincian Kolom I (dengan AKJ & STATUS) */
$detailTitle = "RINCIAN AKUMULASI SISA BON ORDER BELUM REVIEW (Kolom I)";
$detailHdr   = ["No", "SalesOrder", "OrderLine", "AKJ", "STATUS"];
$idxDetailTitle = $addStr($detailTitle);
$idxDetailHdr   = array_map($addStr, $detailHdr);

/* Siapkan indeks string untuk setiap baris rincian (SO & AKJ) */
$I_detail_str_idx = []; // [ [idxSO, idxAKJ], ... ]
foreach ($I_details as [$so, $ol, $akj]) {
  $I_detail_str_idx[] = [$addStr($so), $addStr($akj)];
}
...
/* row4 Data utama */
$sheet[] = '<row r="4">';
$sheet[] = '<c r="A4" t="n" s="2"><v>'.$B.'</v></c>';
$sheet[] = '<c r="B4" t="n" s="2"><v>'.$C.'</v></c>';
$sheet[] = '<c r="C4" t="s" s="1"><v>'.$idxPIC.'</v></c>';
$sheet[] = '<c r="D4" t="s" s="1"><v>'.$idxDtxt.'</v></c>';
$sheet[] = '<c r="E4" t="s" s="1"><v>'.$idxEtxt.'</v></c>';
$sheet[] = '<c r="F4" t="s" s="1"><v>'.$idxFtxt.'</v></c>';
$sheet[] = '<c r="G4" t="n" s="2"><v>'.$G.'</v></c>';
$sheet[] = '<c r="H4" t="n" s="2"><v>'.$H.'</v></c>';
$sheet[] = '<c r="I4" t="n" s="2"><v>'.$I.'</v></c>';
$sheet[] = '</row>';

/* row5 spacer */
$sheet[] = '<row r="5"></row>';

/* row6: Judul rincian (merge A6:E6) */
$sheet[] = '<row r="6" ht="20"><c r="A6" t="s" s="5"><v>'.$idxDetailTitle.'</v></c></row>';

/* row7: Header rincian */
$sheet[] = '<row r="7" ht="20" customHeight="1">'
         . '<c r="A7" t="s" s="3"><v>'.$idxDetailHdr[0].'</v></c>'
         . '<c r="B7" t="s" s="3"><v>'.$idxDetailHdr[1].'</v></c>'
         . '<c r="C7" t="s" s="3"><v>'.$idxDetailHdr[2].'</v></c>'
         . '<c r="D7" t="s" s="3"><v>'.$idxDetailHdr[3].'</v></c>'
         . '<c r="E7" t="s" s="3"><v>'.$idxDetailHdr[4].'</v></c>'
         . '</row>';
...
for ($i = 0; $i < count($I_details); $i++) {
  [$so, $ol, $akj] = $I_details[$i];
  [$idxSO, $idxAKJ] = $I_detail_str_idx[$i];

  $sheet[] = '<row r="'.$rowNo.'">'
           . '<c r="A'.$rowNo.'" t="n" s="2"><v>'.$no.'</v></c>'
           . '<c r="B'.$rowNo.'" t="s" s="2"><v>'.$idxSO.'</v></c>'
           . '<c r="C'.$rowNo.'" t="n" s="2"><v>'.$ol.'</v></c>'
           . '<c r="D'.$rowNo.'" t="s" s="2"><v>'.$idxAKJ.'</v></c>'
           . '<c r="E'.$rowNo.'" s="2"/>' // STATUS kosong
           . '</row>';
  $rowNo++; $no++;
}
...
/* merge: title & header groups (TOTAL 10 MERGE) */
$sheet[] = '<mergeCells count="10">'
         . '<mergeCell ref="A1:I1"/>'
         . '<mergeCell ref="A2:A3"/>'
         . '<mergeCell ref="B2:B3"/>'
         . '<mergeCell ref="C2:C3"/>'
