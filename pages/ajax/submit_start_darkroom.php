...
function processUpdate($con, $no_resep, $expected_status, $new_status, $update_end_time = false) {
    ...
    if ($update_end_time) {
        $update = $con->prepare("UPDATE tbl_preliminary_schedule SET status = ?, sekali_celup = NOW() WHERE no_resep = ?");
    } elseif ($new_status === 'in_progress_darkroom') {
        $update = $con->prepare("UPDATE tbl_preliminary_schedule SET status = ?, darkroom_start = NOW() WHERE no_resep = ?");
    } else {
