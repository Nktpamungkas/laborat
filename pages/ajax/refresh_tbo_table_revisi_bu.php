...
// Snapshot terakhir MySQL per code
$lastMySQLByCode = [];
$qMy = mysqli_query($con, "SELECT * FROM approval_bon_order WHERE is_revision=1 ORDER BY id DESC");
if ($qMy) {
    while ($r = mysqli_fetch_assoc($qMy)) {
        $codeKey = strtoupper(trim($r['code']));
        if (!isset($lastMySQLByCode[$codeKey])) $lastMySQLByCode[$codeKey] = $r;
    }
}

// Ambil dari DB2 (tanpa NOT IN)
$sqlTBO = "
WITH base AS (
    SELECT
        isa.CODE                                AS CODE,
...
// echo <tr> rows saja (dipakai DataTables .rows.add($(data)))
