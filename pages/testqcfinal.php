                    ...
                    <div class="col-lg-12 overflow-auto table-responsive" style="overflow-x: auto;">
                        <table id="Table-sm" class="table table-sm display compact" style="width: 100%;">
                            ...
                            <tbody>
								...
								$sql = mysqli_query($con,"SELECT * FROM tbl_test_qc WHERE sts_laborat <> 'Approved Full' AND suffix LIKE '%$Suffix%' AND jenis_testing LIKE '%$JnsTesting%' AND no_warna LIKE '%$Nowarna%' AND warna LIKE '%$Warna%' AND no_item LIKE '%$Item%' AND no_counter LIKE '%$CounterNo%' ORDER BY id ASC");	
								}else{
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td valign="center" align="center"><?php echo $r['suffix']; ?><hr class="divider">
...
<!--											<a href="pages/cetak/form_permintaan.php?idkk=<?php echo $r['id']; ?>" id='<?php echo $r['id'] ?>' class="btn btn-xs btn-primary"  target="_blank" title="Form Permintaan Test"> <i class="fa fa-file" aria-hidden="true"></i></a>-->
                                        ...
                                        <td valign="center" class="13"><?php echo $r['created_by']; ?><hr class="divider"><span class="label <?php if($r['sts']=="normal"){ echo "label-warning"; }else{ echo "label-danger blink_me"; } ?>"><?php echo $r['sts']; ?></span></td>
                                        <td valign="center" class="13"><span class="label <?php if($r['sts_laborat']=="Open"){ echo "label-info"; }else if($r['sts_laborat']=="In Progress"){ echo "label-success"; }else{ echo "label-primary"; } ?>"><?php echo $r['sts_laborat']; ?></span><hr class="divider"><em><?php echo $r['note_laborat']; ?></em><a href="#" id='<?php echo $r['id'] ?>' class="note_laborat_edit"> <i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
                                        <td align="left" valign="middle" class="13"><span class="label <?php if($r['sts_qc']=="Tunggu Kain"){ echo "label-primary"; }else if($r['sts_qc']=="Sudah Terima Kain"){ echo "label-success"; }else if($r['sts_qc']=="Kain Sudah diTes"){ echo "label-info"; }else if($r['sts_qc']=="Kain Bisa Diambil"){ echo "label-danger blink_me"; }else{ echo "label-warning"; } ?>"><?php echo $r['sts_qc']; ?></span><hr class="divider">
                                        <em>Diterima oleh: <strong><?php echo $r['diterima_oleh']; ?></strong><br>
                                        Penerima: <strong><?php echo $r['nama_penerima']; ?></strong></em><br><em><?php echo $r['note_qc']; ?></em></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
