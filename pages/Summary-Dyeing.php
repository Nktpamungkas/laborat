...
<div class="toolbar">
  <button id="addRowDye" class="btn btn-primary">+ Tambah Baris</button>
  ...
  <button id="exportXlsDye" class="btn btn-success"><i class="fa fa-file-excel-o"></i> Export Excel</button>
  <button id="exportPdfDye" class="btn btn-danger"><i class="fa fa-file-pdf-o"></i> Export PDF</button>
</div>

<div id="gridDye"></div>

<!-- Modal DETAIL SUFFIX (2 tabel) -->
<div id="dyeModalBackdrop" class="modal-backdrop"></div>
<div id="dyeModal" class="modal-shell" role="dialog" aria-modal="true" aria-labelledby="dyeTitle">
  ...
  <div class="modal-footer">
    <button id="dyeClose" class="btn btn-default">Tutup</button>
  </div>
</div>
...
<script>
  ...
  var columnsDye = [
    ...
    { title:"SUFFIX", field:"_suffix_btn", width:w(160), headerSort:false, hozAlign:"center",
      ...
      cellClick: function(e, cell){ openDyeModal(cell.getRow()); },
      download:false
    },
  ...
  /* ===== Table ===== */
  var tableDye = new Tabulator("#gridDye", {
    layout:'fitData',
    columnMinWidth:40,
  ...
  /* ===== Modal DETAIL (pakai suggest_summary_dyeing.php) ===== */
  var dyeBackdrop = document.getElementById('dyeModalBackdrop');
  ...
  async function openDyeModal(row){
    var d = row.getData();
    ...
    try{
      var url = 'pages/ajax/suggest_summary_dyeing.php?date='
      ...
      dyeRangeEl.textContent = 'Periode: ' + json.range_start + ' s/d ' + json.range_end;
      var det = json.detail || {poly:[], cotton:[]};
  ...
  async function fetchSuggest(dateStr, shiftStr){
    ...
    var url = 'pages/ajax/suggest_summary_dyeing.php?date='+encodeURIComponent(dateStr)+'&shift='+shiftStr;
    try{
  ...
  async function saveRowDye(row, btn){
    ...
    var url = isUpdate ? 'pages/ajax/update_row_summary_dyeing.php'
                       : 'pages/ajax/save_row_summary_dyeing.php';
  ...
  async function deleteRowDye(row, btn){
    var data = row.getData();
    if (!confirm('Hapus baris ini?')) return;
    if (data.id){
      if (btn) setBtnSpinner(btn, true);
      var p = new URLSearchParams(); p.append('id', String(data.id));
      var resp = await postForm('pages/ajax/delete_row_summary_dyeing.php', p);
      if (btn) setBtnSpinner(btn, false);
  ...
  async function loadData(from, to){
    try{
      ...
      var url = 'pages/ajax/get_summary_dyeing.php' + (qs.length? ('?'+qs.join('&')) : '');
      var res = await fetch(url);
      var json = await res.json();
      if (json && json.ok){
        var rows = json.data || [];
        tableDye.setData(rows);
        tableDye.clearFilter(true);
        if (from || to){
          tableDye.setFilter(function(d){
            var t = d.tgl || '';
  ...
  document.getElementById('exportXlsDye').addEventListener('click', function () {
    ...
    ws['!cols'] = [{wch:14},{wch:12},{wch:14},{wch:14},{wch:16},{wch:18}];
  ...
  document.getElementById('exportPdfDye').addEventListener('click', function(){
    ...
    var rowTop = [
      ...
      {content:'TOTAL KLOTER', colSpan:2, styles:{halign:'center'}},
