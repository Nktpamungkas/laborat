...
<script>
    ...
    $(document).ready(function () {
        // MODULE RFID
            let filteredDarkRoomEnd = [] // For submit payload
            ...
            socket.on('register', ({
                ...
                tags,
                epcTable,
                filteredData: filteredDarkRoomEnd,
                globalData: darkRoomEnd,
                checkFn: (item, docId) => {
                    const existsOnSelected = [...repeatList, ...endList, ...holdList].includes(docId.trim())
                    if (existsOnSelected) {
            ...
            $('#epcTable').on('click', '.remove-row', function () {
                ...
                // Hapus juga dari filteredDarkRoomEnd
                filteredDarkRoomEnd = filteredDarkRoomEnd.filter(item => item.no_resep.trim() !== noResep);
            });

            $('#submitBtnRFID').on('click', function () {
                filteredDarkRoomEnd.map((item) => {
        ...
        $('#scanInput').on('keypress', function (e) {
            if (e.which === 13) {
                ...
                // Validasi
                $.get(`pages/ajax/validate_scan_dr_end.php?no_resep=${scanned}`, function(response) {
                    if (!response.valid) {
                        Swal.fire({
                            icon: 'error',
