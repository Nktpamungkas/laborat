...
function nourut()
{
	include "koneksi.php";
    $format = "R".date("ym");
    $sql=mysqli_query($con,"SELECT no_resep FROM tbl_tempelan WHERE substr(no_resep,1,5) like '%".$format."%' ORDER BY id DESC LIMIT 1 ") or die(mysqli_error());
...
salesorders.buyerid,processcontrolbatches.lotno,productcode,productmaster.color,colorno,description,productmaster.weight,cuttablewidth,
    ...
    if (isset($_POST['simpan'])) {
        ...
        $qry=mysqli_query($con,"INSERT INTO tbl_tempelan SET
		...
		ck_bleaching_dye='$_POST[ckpro7]',
		tgl_buat=now(),
		tgl_update=now()
		");
        if ($qry) {
            echo "<script>alert('Data Tersimpan');window.location.href='?p=form-tempelan-cetak&idkk=$_POST[no_resep]';</script>";
            echo "<script>window.location.href='#tab_2';</script>";
        } else {
...
?>
							...
							<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form1">
								<div class="box-body">
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input name="no_order" type="text" class="form-control" id="order" onchange="window.location='?p=Form-Tempelan&idk='+this.value" value="<?php if ($_GET['idk']!="") {
    echo $_GET['idk'];
    ...
    echo $ssr1['partnername']."/".$ssr2['partnername']; } else { echo $rw['langganan']; }?>" >
									...
									<div class="form-group">
										...
										<div class="col-sm-7">
											<select name="no_item1" class="form-control select2" id="no_item1" onchange="window.location='?p=Form-Tempelan&idk=<?php echo $_GET['idk'];?>&iditem='+this.value" required>
												<?php
...
WHERE  JobOrders.documentno='".$_GET['idk']."' and processcontrolJO.pcid='".$r1['pcid']."'");
                ...
                ?>
									...
									<div class="form-group">
										<label for="kadar_air" class="col-sm-2 control-label">Kadar Air</label>
										<div class="col-sm-2">
											<input name="kadar_air" type="text" class="form-control" id="kadar_air" placeholder="Kadar Air">
										</div>
									</div>
									<div class="form-group">
										<label for="kd_resep" class="col-sm-2 control-label">Kode Resep</label>
										<div class="col-sm-3">
											<input name="kd_resep" type="text" class="form-control" id="kd_resep" placeholder="Kode Resep">
										</div>
									</div>
									<div class="form-group">
										...
										<div class="col-sm-3">
											<div class="checkbox">
												...
												<label>
													...
													Bleaching LAB</label>
												<label>
													<input type="checkbox" value="1" name="ckpro7">
													Bleaching DYE</label>
												<label>
						...
						<div class="tab-pane" id="tab_2">
							<?php
   $datatmp=mysqli_query($con,"SELECT * FROM tbl_tempelan WHERE no_order='$_GET[idk]' ORDER BY id DESC");
    $no=1;
    $n=1;
    $c=0;
     ?>
							<table width="100%" border="0" id="example2" class="table table-hover display" cellspacing="0">
								...
								<tbody>
									<?php
      $col=0;
   while ($rowd=mysqli_fetch_array($datatmp)) {
       $bgcolor = ($col++ & 1) ? 'gainsboro' : 'antiquewhite';
       $sjml=mysqli_query($con," SELECT count(*) as jml FROM `tbl_tempelan_detail` a
	   INNER JOIN `tbl_tempelan` b ON b.id=a.id_tempelan
	   WHERE b.no_resep='$rowd[no_resep]' ");
       $row=mysqli_fetch_array($sjml); ?>
									<tr bgcolor="<?php echo $bgcolor; ?>">
										<td><a href="?p=form-tempelan-detail&noresep=<?php echo $rowd['no_resep']; ?>&id=<?php echo $rowd['id']; ?>">
										...
										<td>
											<div align="center"><a href="pages/cetak/tempelan.php?idkk=<?php echo $rowd['no_resep']; ?>" class="btn btn-info" target="_blank"><i class="fa fa-print"></i> Cetak</a> </div>
										</td>
									</tr>
									<?php
   } ?>
