<?php
    ...
    function masuk_bulan_ini($head_code, $start, $end, $jenis_matching)
    {
        ...
        FROM tbl_matching a 
        left join tbl_status_matching b on a.no_resep = b.idm
        where substring(a.no_resep, 1,2) = '$head_code' and DATE_FORMAT(a.tgl_buat ,'%Y-%m-%d') >= '$start' and DATE_FORMAT(a.tgl_buat , '%Y-%m-%d') <= '$end' and jenis_matching = '$jenis_matching'
        group by a.jenis_matching, headcd
    ...
    function belum_keluar_bulan_ini($head_code, $start, $end, $jenis_matching)
    {
        ...
        FROM tbl_matching a 
        left join tbl_status_matching b on a.no_resep = b.idm
        where substring(a.no_resep, 1,2) = '$head_code' and DATE_FORMAT(a.tgl_buat ,'%Y-%m-%d') >= '$start' and DATE_FORMAT(a.tgl_buat , '%Y-%m-%d') <= '$end' 
    ...
    function keluar_bulan_ini($head_code, $start, $end, $jenis_matching)
    {
        ...
        FROM tbl_matching a 
        left join tbl_status_matching b on a.no_resep = b.idm
        where substring(a.no_resep, 1,2) = '$head_code' and DATE_FORMAT(b.approve_at ,'%Y-%m-%d') >= '$start' and DATE_FORMAT(b.approve_at , '%Y-%m-%d') <= '$end' 
    ...
    function get_sum_by_matcher($final_matcher, $head_code, $start, $end, $jenis_matching)
    {
        ...
        where a.final_matcher = '$final_matcher' and substring(b.no_resep, 1,2) = '$head_code' and DATE_FORMAT(a.approve_at ,'%Y-%m-%d') >= '$start' and DATE_FORMAT(a.approve_at , '%Y-%m-%d') <= '$end' 
    ...
    function get_summary($start, $end)
    {
        ...
        from tbl_status_matching a
        join tbl_matching b on a.idm = b.no_resep
        where DATE_FORMAT(a.approve_at ,'%Y-%m-%d') >= '$start' and DATE_FORMAT(a.approve_at , '%Y-%m-%d') <= '$end' and a.approve = 'TRUE'");
        $data = mysqli_fetch_array($sql_);
        return $data['summary'];
    }
?>
