<?php
// Approve-Revisi-Bon-Order.php
include "koneksi.php";
require_once __DIR__ . "/lib/revisi_compare.php"; // punya fungsi revisionsDiffer($db2Row, $mysqlSnap)

/**
 * (Opsional) Aktifkan perbandingan LINE (orderline) untuk ikut menentukan "siap approve".
 * Jika true: selain header revisi, sistem juga cek snapshot line di table `line_revision`.
 */
$ENABLE_LINE_DIFF = true;

/* --------------------------------------------------------------------------
 * Helper: ambil snapshot terakhir per CODE dari MySQL (tabel approval_bon_order)
 * -------------------------------------------------------------------------- */
$sqlSnap = "
SELECT a.*
...
/* --------------------------------------------------------------------------
 * Helper (LINE): ambil kondisi revisi per ORDERLINE dari DB2 (disamakan
 *  dengan Approved_get_order_detail_revision_json.php)
 * return: [ orderline => [revisic, revisi2..5, revisin, drevisi2..5, revisi1date..5date] ]
 * -------------------------------------------------------------------------- */
function fetch_db2_lines($conn1, $codeUpper)
{
    $sql = "
    SELECT
        ORDERLINE,
    ...
    HAVING COALESCE(
        ...
        NULLIF(TRIM(MAX(Revisid1)), ''),
        NULLIF(TRIM(MAX(Revisid2)), ''),
        NULLIF(TRIM(MAX(Revisid3)), ''),
        NULLIF(TRIM(MAX(Revisid4)), '')
    ) IS NOT NULL
    ORDER BY ORDERLINE
    ";
    ...
    if ($ok) {
        while ($r = db2_fetch_assoc($stmt)) {
            ...
            $out[$ol] = [
                ...
                'drevisi2'    => (string)($r['REVISID1'] ?? ''),
                'drevisi3'    => (string)($r['REVISID2'] ?? ''),
                'drevisi4'    => (string)($r['REVISID3'] ?? ''),
                'drevisi5'    => (string)($r['REVISID4'] ?? ''),
...
/* --------------------------------------------------------------------------
 * Helper (LINE): ambil snapshot terakhir per ORDERLINE dari MySQL
 * -------------------------------------------------------------------------- */
function fetch_mysql_lines($con, $codeUpper)
{
    ...
    $sql = "
        ...
        JOIN (
            SELECT code, orderline, MAX(id) AS max_id
            FROM line_revision
            WHERE code = '{$codeEsc}'
            GROUP BY code, orderline
        ) m ON m.max_id = lr.id
        WHERE lr.code = '{$codeEsc}'
    ";
    ...
    if ($res) {
        while ($r = mysqli_fetch_assoc($res)) {
            ...
            $out[$ol] = [
                ...
                'revisin'     => (string)($r['revisin']     ?? ''),
                'drevisi2'    => (string)($r['drevisi2']    ?? ''),
                'drevisi3'    => (string)($r['drevisi3']    ?? ''),
                'drevisi4'    => (string)($r['drevisi4']    ?? ''),
                'drevisi5'    => (string)($r['drevisi5']    ?? ''),
                'revisi1date' => (string)($r['revisi1date'] ?? ''),
                'revisi2date' => (string)($r['revisi2date'] ?? ''),
                'revisi3date' => (string)($r['revisi3date'] ?? ''),
                'revisi4date' => (string)($r['revisi4date'] ?? ''),
...
/* --------------------------------------------------------------------------
 * Helper: bandingkan LINE DB2 vs MySQL
 * return true jika ADA perbedaan (atau ada orderline baru/hilang)
 * -------------------------------------------------------------------------- */
function has_line_diff($conn1, $con, $codeUpper)
{
    $db2 = fetch_db2_lines($conn1, $codeUpper);
    $my  = fetch_mysql_lines($con,  $codeUpper);

    // kalau MySQL belum punya snapshot line untuk code ini, anggap TIDAK usah dibandingkan (ikut rule: data baru)
    if (empty($my)) return false;

    // Jika DB2 mengandung orderline yang tidak ada di MySQL → beda
    foreach ($db2 as $ol => $v) {
        if (!isset($my[$ol])) return true;
        foreach ($v as $k => $valDB2) {
            $valMY = (string)($my[$ol][$k] ?? '');
            if (trim((string)$valDB2) !== trim((string)$valMY)) {
                return true;
            }
        }
    }

    // Jika MySQL punya orderline yang tidak ada lagi di DB2 → juga kita anggap beda
    foreach ($my as $ol => $v) {
        if (!isset($db2[$ol])) return true;
    }
...
/* --------------------------------------------------------------------------
 * 2) Ambil kandidat "siap approve" dari DB2 (header)
 * -------------------------------------------------------------------------- */
$sqlTBO = "
WITH base AS (
    SELECT
...
/* --------------------------------------------------------------------------
 * 3) Bangun list untuk TBO (siap approve) dengan logika:
 *    - kalau belum ada snapshot MySQL (kode baru) => MASUK TBO
 *    - kalau ada snapshot:
 *        * bila revisionsDiffer(header) => MASUK TBO
 *        * ATAU (opsional) bila has_line_diff(...) => MASUK TBO
 * -------------------------------------------------------------------------- */
$tboRows = [];
while ($row = db2_fetch_assoc($resultTBO)) {
    ...
    if ($headerDiff || $lineDiff) {
        $tboRows[] = $row;
    }
}

/* --------------------------------------------------------------------------
 * 4) Approved (riwayat): tampilkan 1 baris terbaru per code,
 *    dan EXCLUDE code yang sedang muncul di TBO (jangan double)
 * -------------------------------------------------------------------------- */
$excludeList = "";
if (!empty($tboRows)) {
    $codes = [];
...
<script>
  // ==== Modal Detail Order (table di modal) ====
  ...
  $(document).ready(function(){
    ...
    function reloadApprovedTable(isRevision=1){
      $.get("pages/ajax/refresh_approved_table_revisi.php", { is_revision:isRevision }, function (html) {
        ...
        // rapikan kolom & kunci sort awal biar tampilan konsisten
        approvedTable.columns.adjust().order([0,'desc']).draw(false);
      });
    }

    // (opsional) badge counter kalau ada
    function toInt(x){
      try {
        if (typeof x === 'string' && x.trim().startsWith('{')) {
          const obj = JSON.parse(x);
          for (const k in obj) { if (Object.hasOwn(obj,k) && !isNaN(parseInt(obj[k],10))) return parseInt(obj[k],10); }
        }
      } catch(e){}
      const n = parseInt(String(x).replace(/[^\d-]/g,''),10);
      return isNaN(n)?0:n;
    }
    function updateBadge(tboCount, tboRevisiCount){
    ...
    function refreshTBOCount(){
      $.get('pages/ajax/get_total_tbo.php', function(d1){
        $.get('pages/ajax/get_total_tbo_revisi.php', function(d2){
          updateBadge(toInt(d1), toInt(d2));
        });
      });
    }
      ...
      Swal.fire({title:`${action} Bon Order?`, text:`Kode: ${code} | PIC: ${pic}`, icon:'question', showCancelButton:true, confirmButtonText:action, cancelButtonText:'Batal'})
        .then((result)=>{
          ...
          $.post("pages/ajax/approve_bon_order_lab.php", {
            ...
            // Urutan: update pending dulu → baru riwayat
            reloadTboTable();
    ...
    // Delegation untuk tombol dalam table (karena baris di-reload)
  ...
  // ===== Handler tombol “Detail Revisi” di tabel dalam modal detailApprovedTable =====
  $(document).on('click', '#detailApprovedTable .revisi-btn', function(){
    ...
    var $tbody = $('#revisionTable tbody'); $tbody.empty();
    rows.forEach(function(r){
      var d=String(r.det||'').trim(); if(d==='')return;
      var $tr=$('<tr/>');
      $tr.append($('<td/>').text(String(r.cat||'').trim()||'-'));
      $tr.append($('<td/>').text(d));
      $tr.append($('<td/>').text(String(r.dt||'').trim()));
