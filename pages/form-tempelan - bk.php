...
{
    $format = "R".date("ym");
    $sql=mysql_query("SELECT no_resep FROM tbl_tempelan WHERE substr(no_resep,1,5) like '%".$format."%' ORDER BY no_resep DESC LIMIT 1 ") or die(mysql_error());
...
salesorders.buyerid,processcontrolbatches.lotno,productcode,productmaster.color,colorno,description,productmaster.weight,cuttablewidth,
    ...
    if (isset($_POST[simpan])) {
        ...
        $qry=mysql_query("INSERT INTO tbl_tempelan SET
		...
		");
        if ($qry) {
            echo "<script>alert('Data Tersimpan');window.open('pages/cetak/tempelan.php?idkk=$_POST[no_resep]');</script>";
...
?>
    ...
    echo $ssr1['partnername']."/".$ssr2['partnername']; } else { echo $rw[langganan]; }?>" >
									...
									<div class="form-group">
										...
										<div class="col-sm-3">
											<select name="no_item1" class="form-control" id="no_item1" onchange="window.location='?p=Form-Tempelan&idk=<?php echo $_GET[idk];?>&iditem='+this.value" required>
												<?php
...
WHERE  JobOrders.documentno='$_GET[idk]' and processcontrolJO.pcid='$r1[pcid]'", $conn);
                ...
                ?>
						...
						<div class="tab-pane" id="tab_2">
							<?php
   $datatmp=mysql_query("SELECT * FROM tbl_tempelan WHERE no_order='$_GET[idk]' ORDER BY id DESC");
    $no=1;
    $n=1;
    $c=0;
     ?>
   ...
   while ($rowd=mysql_fetch_array($datatmp)) {
       ...
       $sjml=mysql_query(" SELECT count(*) as jml FROM `tbl_tempelan_detail` a
